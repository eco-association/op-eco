...
function getABI(path: string): ethers.utils.Interface {
  return new ethers.utils.Interface(
    JSON.parse(fs.readFileSync(path, 'utf8')).abi
  )
}
...
const ecoABI: ethers.utils.Interface = getABI(
  `artifacts/@helix-foundation/currency/contracts/currency/ECO.sol/ECO.json`
)

const L1ECOBridgeABI: ethers.utils.Interface = getABI(
  'artifacts/contracts/bridge/L1ECOBridge.sol/L1ECOBridge.json'
)
const L2ECOBridgeABI: ethers.utils.Interface = getABI(
  'artifacts/contracts/bridge/L2ECOBridge.sol/L2ECOBridge.json'
)
const L2ECOABI: ethers.utils.Interface = getABI(
  'artifacts/contracts/token/L2ECO.sol/L2ECO.json'
)
...
async function deployL1Bridge() {
  const bridgeFactory = ethers.ContractFactory.fromSolidity(
    L1ECOBridgeABI,
    l1Wallet
  )
...
async function initializeL2ECO() {
  const l2eco = new ethers.Contract(
    L2_ECO_ADDRESS,
    L2ECOABI.fragments,
    l2Wallet
  )
  const tx = await l2eco.initialize(
    L1_ECO_ADDRESS,
    L2_BRIDGE_ADDRESS,
    l2Wallet.address
  )
...
async function approveAndDeposit() {
  ...
  const tx2 = await l1bridge.depositERC20(
    L1_ECO_ADDRESS,
    L2_ECO_ADDRESS,
    amount,
    l2gas,
...
async function proveWithdrawal() {
  const withdrawalTx1 = {
    hash: '0x97b9c80097b77d5d79813b5367e515d83c25d21185550ed7b9d9c51348b016c0',
  }
  await crossChainMessenger.waitForMessageStatus(
    withdrawalTx1.hash,
    MessageStatus.READY_TO_PROVE
  )
  const withdrawalTx2 = await crossChainMessenger.proveMessage(
    withdrawalTx1.hash
  )
...
async function finalizeWithdrawal() {
  ...
  const withdrawalTx3 = await crossChainMessenger.finalizeMessage(
    withdrawalTx1.hash
  )
