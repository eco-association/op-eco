...
describe('L2ECO tests', () => {
  ...
  describe('burning', () => {
    ...
    describe('decreases balance', () => {
      beforeEach(async () => {
        await L2ECO.connect(l2BridgeImpersonator).mint(
          alice.address,
          burnAmount
        )
      ...
      it('on admin call', async () => {
        expect((await L2ECO.balanceOf(alice.address)) === burnAmount).to.be.true
        await L2ECO.connect(l2BridgeImpersonator).burn(
          alice.address,
          burnAmount
        )
        expect((await L2ECO.balanceOf(alice.address)) === 0).to.be.true
  ...
  describe('rebasing', () => {
    ...
    describe('on rebase', () => {
      ...
      beforeEach(async () => {
        ...
        expect((await L2ECO.balanceOf(alice.address)) === aliceBalance).to.be
          .true
      })
      ...
      it('emits an event', async () => {
        await expect(
          ...
          .to.emit(L2ECO, 'NewInflationMultiplier')
      ...
      it('changes balance', async () => {
        ...
        expect((await L2ECO.balanceOf(alice.address)) === 2 * aliceBalance).to
          .be.true
      })
    })
  })
  ...
  describe('transfers', () => {
    ...
    beforeEach(async () => {
      ...
      expect((await L2ECO.balanceOf(alice.address)) === initialAliceBalance).to
        .be.true
    })
    ...
    it('emits base value event', async () => {
      await expect(
        ...
        .to.emit(L2ECO, 'BaseValueTransfer')
        .withArgs(alice.address, bob.address, ethers.utils.parseEther('1000'))
    })
  })
  ...
  describe('role management', () => {
    ...
    describe('minting', () => {
      ...
      it('can add permission', async () => {
        ...
        await L2ECO.connect(l2BridgeImpersonator).updateMinters(
          alice.address,
          true
        )
    ...
    describe('burning', () => {
      ...
      it('can add permission', async () => {
        ...
        await L2ECO.connect(l2BridgeImpersonator).updateBurners(
          bob.address,
          true
        )
      ...
      it('can remove permission', async () => {
        ...
        expect((await L2ECO.balanceOf(alice.address)) === 2 * burnAmount).to.be
          .true
        await L2ECO.connect(l2BridgeImpersonator).burn(
          alice.address,
          burnAmount
    ...
    describe('rebasing', () => {
      ...
      it('can add permission', async () => {
        ...
        await L2ECO.connect(l2BridgeImpersonator).updateRebasers(
          alice.address,
          true
        )
    ...
    describe('admin', () => {
      it('can change admin', async () => {
        ...
        await L2ECO.connect(l2BridgeImpersonator).updateTokenRoleAdmin(
          alice.address
        )

        // can no longer edit roles
        await expect(
          L2ECO.connect(l2BridgeImpersonator).updateMinters(
            alice.address,
            false
          )
        ).to.be.revertedWith(ERROR_STRINGS.INVALID_TOKEN_ROLE_ADMIN)
